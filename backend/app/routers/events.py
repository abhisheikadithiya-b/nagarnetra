...
def ingest_event(event: IngestEvent, db: Session = Depends(get_db)):
    ...
    # 2. Coordinate Sanity & Geofence Check (Chennai Greater Metropolitan Area)
    if not (settings.GEOFENCE_MIN_LAT <= event.lat <= settings.GEOFENCE_MAX_LAT and settings.GEOFENCE_MIN_LON <= event.lon <= settings.GEOFENCE_MAX_LON) and event.source != "sim":
        return IngestResponse(
            status="rejected",
            event_id=event.id,
            dedup_action="geofence_out_of_bounds",
            message=f"Coordinates ({event.lat}, {event.lon}) fall outside {settings.CITY_NAME} municipal telemetry bounds."
        )
